...
class Helper:
    def __init__(self):
        self.level_sensor = LevelSensor('water sensor')
...
class TestSump(TestCase):
    ...
    def test_percentage_changed_parameterized(self):
        ...
        for water_level, percentage in params.items():
            self.helper.level_sensor.get_level = MagicMock(return_value=water_level)
    ...
    def test_unexpected_water_level_error_raised(self):
        ...
        for level in too_high + too_low:
            self.helper.level_sensor.get_level = MagicMock(return_value=level)
    ...
    def test_update(self):
        self.helper.level_sensor.get_level = MagicMock(return_value=21)
        self.helper.temperature_detector.temperature_breakdown = MagicMock(return_value=([1, 2], [3, 4], 5))
        sump_update = SumpUpdate([1, 2], [3, 4], 5, 21)
        self.assertEqual(sump_update, self.sump.get_update())
